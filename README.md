...
evenly spaced sample points, reads a fixed amount of data at each point, and
...
3. Reads `--sample-mib` MiB at each position and records the elapsed time.
...
Options:
  ...
  --sample-mib MIB  Data to read at each sample point, in MiB  (default: 200)
...
```
    ...
    2   0.50% offset    931.32 MiB read    200.00 MiB in   0.124s   1612.90 MiB/s
  ...
```
...
- On small devices the sample size is clamped to the device size.
